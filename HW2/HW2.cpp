        ...
        for(string::iterator it_str=it_i->begin();it_str!=it_i->end();it_str++)
        {
            if(*it_str=='/')
            {
                ...
                while(getline(buffer1,buffer2,'/'))
                {
                    ...
                    {
                        if(buffer2.compare("_")==0)
                        {
                            cheklist.clear();
                            cheklist.push_back("_");
                            break;
                        }
...
bool compare_candidate_unique(sentence_location& first,sentence_location& second)
{
    if(first.location!=second.location)
        return false;
    else if(first.frequency_array!=second.frequency_array)
        return false;
    else
        return true;
}
...
int main(int argc, char *argv[])
{
    ...
    while(getline(cin,query))
    {
        ...
        else if(candidate.size()<=5)
        {
            ...
            sort(candidate.begin(), candidate.end(),sentence_location_compare);
            printf("output: %d\n",_size);
        }
        else
        {
        ...
        unique (candidate.begin(), candidate.end(),compare_candidate_unique);
        if(candidate.size()!=0)
        {
            for(vector<sentence_location>::iterator it_i=candidate.begin();it_i!=candidate.begin()+5&&it_i!=candidate.end();it_i++)
            {

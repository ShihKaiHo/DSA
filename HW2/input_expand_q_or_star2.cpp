...
void expand_star(vector<string> sentence,vector< vector<string> >& expanded_query)
{
    bool is_clear=1;
    ...
    for(vector<string>::iterator it_i=sentence.begin();it_i!=sentence.end();it_i++)
    {
        if((*it_i)=="*")
        {
            ...
            {
                expand_star(replace_star(sentence,it_i-sentence.begin(),i),expanded_query);
            }
            break;
        }
    }
    if(is_clear==1)
    {
        expanded_query.push_back(sentence);
    }
}
...
void expand_or(vector<string>& sentence,vector< vector<string> >& expanded_query)
{
    bool is_clear=1;
    bool is_or_found=0;
    string buffer2;
            ...
            if(*it_str=='/')
            {
                ...
                while(getline(buffer1,buffer2,'/'))
                {
                    if(check_repeat(cheklist,buffer2))
                    {
                        ...
                        buffer3=*it_i;
                        *it_i=buffer2;
                        expand_or(sentence,expanded_query);
                        *it_i=buffer3;
                    }
                }
                break;
    ...
    if(is_clear==1)
    {
        expand_star(sentence,expanded_query);
    }
}
...
void expand_q(vector<string> sentence,vector< vector<string> >& expanded_query)
{
    bool is_clear=1;
    for(vector<string>::iterator it_i=sentence.begin();it_i!=sentence.end();it_i++)
    {
        ...
        {
            is_clear=0;
            expand_q(erase_q(sentence,it_i-sentence.begin()),expanded_query);
            expand_q(erase_word(sentence,it_i-sentence.begin()),expanded_query);
            break;
        }
    }
    if(is_clear==1)
...
int main(int argc, char *argv[])
{
    ...
    t1=clock();
    string query;
    vector< vector<string> > expanded_query;
    while(getline(cin,query))
    {
        vector<string> query_vec(constuct_string_vec(query));
        expand_q(query_vec,expanded_query);

        /*
        for(vector< vector<string> >::iterator it_i=expanded_query.begin();it_i!=expanded_query.end();it_i++)
        {

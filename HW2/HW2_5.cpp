...
bool sentence_location_unique(const sentence_location& first, const sentence_location& second)
{
...
void expand_star(vector<string> sentence,vector<sentence_location>& candidate,vector< vector<unsigned int> >::iterator& it_frequency_array,Node** roots,vector< vector<string> >& check_repeat_query_list)
{
    ...
    if(is_clear==1)
    {
        if(!check_repeat_query(sentence,check_repeat_query_list))
        {
            ...
            vector< sentence_location > buffer2;
            ...
            if(sentence.size()>=2&&sentence.size()<=5)
            {
                ...
                if(buffer2.size()<5)
                {
                    candidate.insert(candidate.end(),buffer2.begin(),buffer2.end());
                }
                else
                {
                    sort(buffer2.begin(),buffer2.end(),sentence_location_compare);
                    unique (buffer2.begin(), buffer2.end(), sentence_location_unique);
                    candidate.insert(candidate.end(),buffer2.begin(),buffer2.begin()+5);
                }
            }
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    string path(argv[1]);
    string path_2=path+"2gm.small.txt";
    fp_2=fopen(path_2.c_str(),"r");
    ...
    string path_3=path+"3gm.small.txt";
    fp_3=fopen(path_3.c_str(),"r");
    ...
    string path_4=path+"4gm.small.txt";
    fp_4=fopen(path_4.c_str(),"r");
    ...
    string path_5=path+"5gm.small.txt";
    fp_5=fopen(path_5.c_str(),"r");
    ...
    while(getline(cin,query))
    {
        ...
        if(candidate.size()!=0)
        {
            ...
            if(string_candidate.size()<=20)
            {
                sort(string_candidate.begin(),string_candidate.end(),string_candidate_comp);
                string_candidate.erase( unique( string_candidate.begin(), string_candidate.end(),string_pos_unique ), string_candidate.end() );
            }
            else
            {
                sort(string_candidate.begin(),string_candidate.end(),string_candidate_comp);
                string_candidate.erase( unique( string_candidate.begin(), string_candidate.begin()+10,string_pos_unique ), string_candidate.end() );
            }

...
void expand_star(vector<string> sentence,vector<sentence_location>& candidate,vector< vector<unsigned int> >::iterator& it_frequency_array,Node** roots)
{
    ...
    if(is_clear==1)
    {
        ...
        if(sentence.size()>=2&&sentence.size()<=5)
        {
            ...
            partial_sort(buffer2.begin(),buffer2.begin()+5,buffer2.end(),sentence_location_compare);
        }
        if(buffer2.size()<=5)
        {
    ...
    while(getline(cin,query))
    {
        ...
        if(candidate.size()!=0)
        {
            ...
            if (string_candidate.size()<5&&string_candidate.size()>1)
                ...
                partial_sort(string_candidate.begin(),string_candidate.begin()+10,string_candidate.end(),string_candidate_comp);
